...
public class AccountDAO implements IAccountDAO {
    ...
    private static final String FIND_ACCOUNT_BY_USERNAME_AND_PASSWORD="SELECT * FROM account WHERE username=? and password=? and status=?";
...
//                String password1 = rs.getString("password");
                Date signupDate = rs.getDate("signup_date");
                LocalDateTime lastlogin = LocalDateTime.parse(rs.getString("last_login"), formatter);
                //Boolean status1 = Boolean.valueOf(rs.getString("status"));
                int idRole = rs.getInt("id_role");
                account = new Account(id, username, password, signupDate, lastlogin, status, idRole);
